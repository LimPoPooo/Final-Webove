...
function processArtAddFrmData(event,serverUrl){
    ...
    fetch(`${serverUrl}/article/`, postReqSettings)  //now we need the second parameter, an object wih settings of the request.
        ...
        .finally(() => window.location.hash=`#articlesM/1/1`);

}



function vymazatNazory(event,id){
    ...
    const query = new Parse.Query(nazory);
// here you put the objectId that you want to delete
    query.get(id).then((object) => {
        object.destroy().then((response) => {
            if (typeof document !== 'undefined')  console.log('Deleted nazory', response);
            window.location.reload(true);
        }, (error) => {
            if (typeof document !== 'undefined') console.error('Error while deleting nazory', error);
            window.location.reload(true);
        });
    });
    ;
